...
static bool	    ledOn = 1;          ///< Is the LED on or off? Used to invert its state (off by default)
...
static bool	    ledOn2 = 1;          ///< Is the LED on or off? Used to invert its state (off by default)
...
static int __init ebbgpio_init(void){
   ...
   ledOn = true;
   ledOn2 = true;
   gpio_request(gpioLED, "sysfs");          // gpioLED is hardcoded, request it
...
// gpio_set_value(gpioLED, ledOn);          // Not required as set by line above (here for reference)
   ...
   gpio_export(gpioButton, false);          // Causes gpio to appear in /sys/class/gpio
   ...
   printk(KERN_INFO "GPIO_TEST: The interrupt request2 result is: %d\n", result2);
   retvalue =  result+result2;
   return retvalue;
}
